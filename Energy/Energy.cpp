...
//@param O2 所需坐标系下原点
//@return 返回变换之后的坐标点
inline Point2f XYTransform2D(Point2f X, Point2f x1, Point2f x2)
{
    Point2f direction_vector = x2 - x1; //坐标系变化方向向量    
    X -= direction_vector;           //新坐标等于原坐标减去方向向量
    return X;
}
...
// @brief 识别总入口函数
// @param oriFrame 原始图像
bool Energy::run(Mat &oriFrame, ArmorPlate &predictArmor)
{
    if (oriFrame.empty())
    {
    ...
    finalHitCalc(predictArmor);

    //角度解算
    if(angle_slover.getAngle(predictArmor, AngleSolverFactory::TARGET_ARMOR))
    {   //进行角度补偿
        Buff_ShootingAngleCompensate(predictArmor);
    ...
    if (energyParams.stm32Data.energy_mode == ENERGY_BIG)
    {
        ...
        else if(armor_center_in_centerR_cord.size() == 2)
        {
            ...
            if(fabs(measure_a_rad) > 3)
            {
                ...
                measure_a_rad = (fabs(measure_a_rad) / measure_a_rad) * 2; //FIXME::？
            }
    ...
    for (size_t i = 0; i < armor_contours_extenal.size(); i++)
    {
        ...
        for (size_t j = 0; j < armor_contours.size(); j++)
        {
            ...
            if (contour_external_size == contour_size)
            {
                ...
                armor_contours.pop_back();  //去除最后一个轮廓
                break;
            }
        }
    }
...
bool Energy::findTargetArmor(Mat &src_bin)
{
    ...
    for (auto armor : armors_rrect)
    {
        ...
        cout << "armor.angle: " <<  armor.angle << endl;
        cout << "target_flow_strip_fan.angle: " <<  target_flow_strip_fan.angle << endl;
        // 相交点尺寸小于4
...
void Energy::finalHitCalc(ArmorPlate &predictArmor)
{
    ...
    predictArmor.predictPoint = predict_point;
    predictArmor.rrect = cv::RotatedRect(predictArmor.predictPoint + (Point2f)ROI_Offset, target_armor.size, target_armor.angle - preAngleTemp);
}

void Energy::Buff_ShootingAngleCompensate(ArmorPlate &predictArmor)
{
    /**
    ...
    */
    ...
    // predictArmor.angle_y += angle_y_compensate_dynamic + angle_y_compensate_static;
    // predictArmor.angle_x += angle_x_compensate_dynamic + angle_x_compensate_static;
}
